...
def load_original_texts(file_path):
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            try:
                # 尝试拆分文本和方面词信息
                text, aspects = line.split('",[')
                text = text[1:]  # 去除文本开头的引号
...
class EnhancedBertAttentionClassifier(nn.Module):
    def __init__(self, hidden_dim=300, num_classes=3, freeze_bert_layers=True, class_weights=None, num_heads=12):
...
# 交叉验证函数
def cross_validate_model(model_class, dataset, test_loader, original_test_data, num_folds=5, batch_size=16,
                         num_epochs=5, class_weights=None, num_heads=12):
...
if __name__ == "__main__":
    ...
    original_test_file = '2014_Laptop_test.txt'
    ...
    cross_validate_model(EnhancedBertAttentionClassifier, train_dataset, test_loader, original_test_data, num_folds=5,
                         batch_size=16, num_epochs=5, class_weights=class_weights, num_heads=12)
